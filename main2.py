...
def monta_listagem_de_cartas(
    ...
    if not cartas:
        ...
        for i, carta in enumerate(cartas, start=1):
            inicio_de_linha = '-' if not numerar else f'{i})'
...
class Jogador:
    ...
    def _escolhe_carta_possivel(
        ...
        while idx_carta is None:
            ...
            if eh_decimal and eh_valor_valido:
                idx_carta = int(idx_carta_str) - 1
        
        return cartas_possiveis[idx_carta]

    def _descartar(self, carta: Carta, carta2b: Carta, monte_descarte: Pilha[Carta]):
        """
        Tira uma carta da mão e coloca no monte de descarte.
        :param carta: Carta selecionada
        :param monte_descarte: Monte de descarte
        """
        self.cartas.remove(carta2b)
    ...
    def jogar(self, monte_compra: Pilha[Carta], monte_descarte: Pilha[Carta]):
        ...
        print(monta_listagem_de_cartas(cartas_ordenadas))

        cartas_possiveis = self.selecionar(topo_descarte)
        while not cartas_possiveis:
        ...
        print(monta_listagem_de_cartas(self.cartas, numerar=True))

        carta_escolhida = self._escolhe_carta_possivel(cartas_possiveis)
        carta_2b_removed = carta_escolhida
        if eh_carta_especial(carta_escolhida):
            if carta_escolhida.tipo in ('coringa', '+4 coringa'):
                cor_coringa = self.escolhe_cor_de_coringa()
                carta_escolhida = carta_escolhida._replace(cor = cor_coringa)

            topo_esta_ativo = True

        self._descartar(carta_escolhida, carta_2b_removed, monte_descarte)
        return monte_compra, monte_descarte
...
#         elif carta_especial.tipo == 'coringa':
#             pass
#         else:
...
#             print("Prox ", idx_proximo_jogador)
