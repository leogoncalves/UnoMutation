...
from uno2 import Jogador, Pilha, Carta
import uno2

class TestUno(unittest.TestCase):
    def setUp(self):
        self.topo_esta_ativo = False
    ...
    def test_jogar_CT2(self, mock_inputs):
        ...
        monte_compra, monte_descarte, topo_esta_ativo = mao.jogar_rodada(
            monte_compra, 
            monte_descarte,
            self.topo_esta_ativo
        )
    ...
    def test_jogar_CTM2(self,mock_inputs):
        ...
        self.assertEqual(len(monte_descarte), 5)
        self.assertEqual(len(mao.cartas), 1)
        self.assertTrue(topo_esta_ativo)
    ...
    def test_ECDC_CTM1(self,mock_inputs):
        ...
        self.assertFalse(sem_cor, cor_escolhida)
        self.assertTrue(cor_escolhida in uno2.CORES_VALIDAS)
        self.assertTrue(sem_cor not in uno2.CORES_VALIDAS)
    ...
    def test_ECDC_CTM2(self, mock_inputs):
        ...
        with self.assertRaises(StopIteration):
            jogador = Jogador([])
            jogador.escolhe_cor_de_coringa()
    ...
    def test_MLC_CTM1(self):
        ...
        mao = Jogador(conjunto_inicial)
        listagem = uno2.montar_listagem_de_cartas(mao.cartas)
        print(listagem)
...
# mut.py --target main2.py --unit-test JogTest.py -m -c --mutation-number NUMERO_MUTANTE

# mut.py --target main2.py --unit-test JogTest.py -m -c

# mut.py --target main2.py --unit-test JogTest.py -m -c --report-html tes.html